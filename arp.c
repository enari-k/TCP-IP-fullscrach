...
arp_cache_alloc(void)
{
    ...
    for (entry = caches; entry < tailof(caches); entry++) {
        ...
        if (!oldest ||
            oldest->timestamp.tv_sec > entry->timestamp.tv_sec ||
            (oldest->timestamp.tv_sec == entry->timestamp.tv_sec && oldest->timestamp.tv_usec > entry->timestamp.tv_usec))
        {
            oldest = entry;
        }
    }
...
arp_init(void)
{
    ...
    if (timer_register(interval, arp_timer) == -1) {
