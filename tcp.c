            ...
            key1.addr != IP_ADDR_ANY)
        {
            if (pcb->state != TCP_STATE_LISTEN)
            {
                if (pcb->mode == TCP_PCB_MODE_SOCKET &&
                    key2.addr != IP_ADDR_ANY &&
                    key2.port != 0)
                {
                    if (pcb->remote.addr != key2.addr ||
                        pcb->remote.port != key2.port)
                    {
                        continue;
                    }
                }
...
tcp_set_timewait_timer(struct tcp_pcb *pcb)
{
    ...
    debugf("start time_wait timer: %d second", TCP_TIMEWAIT_SEC);
}
                    ...
                    ip_endp_t local, ip_endp_t remote)
{
        ...
        if (TCP_FLG_ISSET(flags, TCP_FLG_SYN))
        {
            ...
            else
            {
                ...
                /* ignore: If there are other controls or text in the segment,
                   queue them fo processing after the ESTABLOSHED state has been reached */
            }
        }
        /*
    ...
    case TCP_STATE_TIME_WAIT:
         ...
         * triming off any portions that lie outside the window (including
    ...
    case TCP_STATE_CLOSING:
        if (pcb->snd.una < seg->ack && seg->ack <= pcb->snd.nxt)
        {
            pcb->snd.una = seg->ack;
            tcp_retrans_queue_cleanup(pcb);
            /* ignore: Users should receive positive acknowledgements for buggers
        ...
        case TCP_STATE_CLOSING:
            if (seg->ack == pcb->snd.nxt)
            {
                ...
                /* NOTE: set 2MSL timer, although it is not expicitly stated in the RFC */
...
tcp_timer(void)
{
    ...
    for (pcb = pcbs; pcb < tailof(pcbs); pcb++)
    {
        ...
        {
            if (now.tv_sec > pcb->tw_timer.tv_sec ||
                (now.tv_sec == pcb->tw_timer.tv_sec &&
                 now.tv_usec > pcb->tw_timer.tv_usec))
            {
